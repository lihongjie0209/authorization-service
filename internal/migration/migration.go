...
func ensureSchema(cfg config.Migration, databaseURL string) error {
	...
	return createSchema(db, cfg.Schema)
}
...
func createSchema(db *sql.DB, schema string) error {
	...
	if _, err := tx.Exec(`SELECT pg_advisory_xact_lock(hashtext($1))`, "platform-schema:"+schema); err != nil {
		return fmt.Errorf("lock migration schema %q: %w", schema, err)
	}
	if _, err := tx.Exec(`CREATE SCHEMA IF NOT EXISTS "` + schema + `"`); err != nil {
		return fmt.Errorf("create migration schema %q: %w", schema, err)
	}
	if err := tx.Commit(); err != nil {
		return fmt.Errorf("commit migration schema %q: %w", schema, err)
	}
	return nil
}
