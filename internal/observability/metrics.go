...
func (m *Metrics) ObserveRoutePolicyRefresh(source, status string, started time.Time) {
	if !m.enabled {
		return
	}
	m.RoutePolicyRefresh.WithLabelValues(source, status).Inc()
	m.RoutePolicyRefreshDuration.WithLabelValues(source).Observe(time.Since(started).Seconds())
}
